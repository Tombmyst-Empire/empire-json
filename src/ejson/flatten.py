...
def _flatten_object(
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            flattened.update(_flatten_object(value, object_keys_separator, array_subscript_separator, prefix=make_prefix(key), flatten_arrays=flatten_arrays, no_index_for_arrays=no_index_for_arrays))
        elif flatten_arrays and isinstance(value, list):
            flattened.update(_flatten_array(value, object_keys_separator, array_subscript_separator, prefix=make_prefix(key), flatten_arrays=flatten_arrays, no_index_for_arrays=no_index_for_arrays))
        else:
...
def _flatten_array(
    ...
    for index, value in enumerate(data):
        if isinstance(value, dict):
            flattened.update(_flatten_object(value, object_keys_separator, array_subscript_separator, prefix=make_prefix(index), flatten_arrays=flatten_arrays, no_index_for_arrays=no_index_for_arrays))
        elif flatten_arrays and isinstance(value, list):
            flattened.update(_flatten_array(value, object_keys_separator, array_subscript_separator, prefix=make_prefix(index), flatten_arrays=flatten_arrays, no_index_for_arrays=no_index_for_arrays))
        else:
...
def get_flattener(
    ...
    def flatten(data: JsonType) -> JsonType:
        ...
        if isinstance(data, dict):
            return _flatten_object(data, object_keys_separator, array_subscript_separator, prefix=root_identifier, flatten_arrays=flatten_arrays, no_index_for_arrays=no_index_for_arrays)
        elif flatten_arrays and isinstance(data, list):
            return _flatten_array(data, object_keys_separator, array_subscript_separator, prefix=root_identifier, flatten_arrays=flatten_arrays, no_index_for_arrays=no_index_for_arrays)
        else:
            return data
